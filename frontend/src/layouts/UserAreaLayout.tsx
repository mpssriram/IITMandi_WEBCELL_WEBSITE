...
const navItems = [
    ...
    { to: "/user/projects", label: "Projects" },
    { to: "/user/members", label: "Members" },
];

const READ_NOTIFICATION_STORAGE_KEY_PREFIX = "devcell_read_notifications_v1";

function getReadNotificationStorageKey(scope: string) {
    return `${READ_NOTIFICATION_STORAGE_KEY_PREFIX}:${scope.trim().toLowerCase()}`;
}
...
function getNotificationScope(profile: UserProfile | null) {
    return profile?.firebase_uid?.trim() || (profile?.id ? `id:${profile.id}` : "") || profile?.email?.trim().toLowerCase() || profile?.roll_number?.trim().toLowerCase() || "guest";
}
...
export function UserAreaLayout() {
    ...
    useEffect(() => {
        if (!profile?.firebase_uid && !profile?.id && !profile?.email && !profile?.roll_number) {
            return;
        }

        const scope = getNotificationScope(profile);
        const storageKey = getReadNotificationStorageKey(scope);

        try {
            const storedReadIds = window.localStorage.getItem(storageKey);
            if (!storedReadIds) {
                setReadNotificationIds([]);
                return;
            }

            const parsed = JSON.parse(storedReadIds) as unknown;
        ...
        setReadNotificationIds([]);
    }, [profile?.firebase_uid, profile?.id, profile?.email, profile?.roll_number]);
    ...
    useEffect(() => {
        if (!isNotificationsPanelOpen || !notifications.length) {
            return;
        }
    ...
    }, [isNotificationsPanelOpen, notifications, profile?.firebase_uid, profile?.id, profile?.email, profile?.roll_number]);
    ...
    const openNotificationTarget = (notificationId: string, href: string) => {
        const scope = getNotificationScope(profile);
        const storageKey = getReadNotificationStorageKey(scope);
        ...
        <div className="relative min-h-screen text-white">
                ...
                <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:px-8">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="flex min-w-0 items-center gap-3">
                            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-[1.15rem] border border-cyan-300/25 bg-cyan-400/10 text-sm font-semibold text-cyan-50 shadow-[0_18px_44px_-24px_rgba(34,211,238,0.75)]">
                                WC
                            </div>
                            <div className="min-w-0">
                                <p className="text-[11px] uppercase tracking-[0.24em] text-cyan-100/70">IIT Mandi Dev Cell</p>
                                <div className="mt-1 flex min-w-0 flex-wrap items-center gap-2">
                                    <span className="rounded-full border border-cyan-300/20 bg-cyan-400/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-100/85">
                                        Workspace
                                    </span>
                        ...
                        <div className="flex items-center gap-2 sm:gap-3">
                            <button
                                ...
                                className={`relative inline-flex h-11 w-11 items-center justify-center rounded-2xl border bg-white/5 text-slate-200 transition hover:border-cyan-300/35 hover:bg-cyan-400/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 focus-visible:ring-offset-ink-950 ${isNotificationsPanelOpen ? "border-cyan-300/40" : "border-white/10"
                                    }`}
                            ...
                            <button
                                type="button"
                                onClick={openProfile}
                                className="inline-flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-2.5 py-1.5 text-left transition hover:border-cyan-300/35 hover:bg-cyan-400/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 focus-visible:ring-offset-ink-950"
                            >
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className={`inline-flex h-11 items-center gap-2 rounded-2xl border bg-white/5 px-3 text-sm font-semibold text-slate-200 transition hover:border-cyan-300/35 hover:bg-cyan-400/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 focus-visible:ring-offset-ink-950 ${isQuickMenuOpen ? "border-cyan-300/40" : "border-white/10"
                                        }`}
                                ...
                                <div
                                    ref={quickMenuRef}
                                    className={`absolute right-0 top-[3.2rem] z-50 w-60 rounded-2xl border border-cyan-300/20 bg-[linear-gradient(145deg,rgba(6,17,35,0.98),rgba(4,10,23,0.96))] p-2 shadow-[0_22px_70px_-28px_rgba(8,145,178,0.8)] transition-all ${isQuickMenuOpen ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-1 opacity-0"
                                        }`}
                                    role="menu"
                                ...
                                >
                                    ...
                                    >
                                        <span className="grid h-7 w-7 place-items-center rounded-lg border border-cyan-300/20 bg-cyan-400/10 text-cyan-200">D</span>
                                        Dashboard
                                    </button>
                                    <button
                                        type="button"
                    ...
                    <nav className="flex flex-wrap gap-2">
                        {navItems.map((item) => (
                            <NavLink
                                ...
                                to={item.to}
                                className={({ isActive }) =>
                                    `inline-flex min-h-10 items-center rounded-full border px-4 py-2 text-sm font-medium transition ${isActive
            ...
            <div className="pointer-events-none fixed inset-x-0 top-20 z-50 mx-auto flex w-full max-w-7xl justify-end px-4 sm:px-6 lg:px-8">
                <div
                    ...
                    className={`pointer-events-auto w-full max-w-sm rounded-[1.5rem] border border-cyan-300/25 bg-[linear-gradient(145deg,rgba(6,17,35,0.98),rgba(4,10,23,0.96))] p-4 shadow-[0_28px_90px_-32px_rgba(8,145,178,0.7)] backdrop-blur-xl transition-all duration-200 ${isNotificationsPanelOpen ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-3 opacity-0"
                        }`}
                    role="dialog"
                        ...
                        {notifications.map((item) => {
                            ...
                            return (
                                <button
                                    key={item.id}
                                    type="button"
                                    onClick={() => openNotificationTarget(item.id, item.href)}
                                ...
                                >
                                    <div className="flex items-start justify-between gap-2">
                                        ...
                                        <span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${isUnread ? "bg-cyan-300 shadow-[0_0_14px_rgba(103,232,249,0.9)]" : "bg-slate-500/70"}`} />
                                    </div>
                                    <div className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-cyan-100/80 transition group-hover:text-cyan-50">
                                        {isUnread ? "Open and mark read" : "Opened"}
