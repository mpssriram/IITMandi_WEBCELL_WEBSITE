...
export function Hyperspeed({ effectOptions = DEFAULT_EFFECT_OPTIONS, className = "", density }) {
    ...
    useEffect(() => {
        ...
        class CarLights {
            ...
            init() {
                ...
                const aMetrics = [];
                const aColor = [];
                const colors = this.colors.map((color) => new THREE.Color(color));
        ...
        class LightsSticks {
            ...
            init() {
                ...
                const stickOffset = options.length / Math.max(totalSticks - 1, 1);
                const aOffset = [];
                const aColor = [];
                const aMetrics = [];
                const colors = Array.isArray(options.colors.sticks)
        ...
        const options = {
            ...
            ...effectOptions,
            totalSideLightSticks: density ?? effectOptions.totalSideLightSticks ?? DEFAULT_EFFECT_OPTIONS.totalSideLightSticks,
            lightPairsPerRoadWay: density ? Math.max(24, density * 2) : effectOptions.lightPairsPerRoadWay ?? DEFAULT_EFFECT_OPTIONS.lightPairsPerRoadWay,
    ...
    return <div ref={hyperspeed} className={`hyperspeed-root ${className}`.trim()} />;
}
