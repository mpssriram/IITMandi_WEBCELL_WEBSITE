...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className, activeIndex, onItemClick }) => {
  ...
  const containerClassName = className || 'gap-[5em] grid-cols-2 md:grid-cols-3 py-[3em]';

  return (
    <div className={`grid mx-auto overflow-visible ${containerClassName}`}>
      {items.map((item, index) => (
        <button
          ...
          className={`relative bg-transparent outline-none border-none cursor-pointer w-[4.5em] h-[4.5em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
            ...
            activeIndex === index ? '[transform:translateY(-0.1em)]' : ''
          }`}
        >
          <span
          ...
          <span
            className={`absolute top-0 left-0 w-full h-full rounded-[1.25em] bg-[hsla(0,0%,100%,0.15)] transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] origin-[80%_50%] flex backdrop-blur-[0.75em] [-webkit-backdrop-filter:blur(0.75em)] [-moz-backdrop-filter:blur(0.75em)] [will-change:transform] transform group-hover:[transform:translate3d(0,0,2em)] ${
              activeIndex === index ? 'ring-2 ring-cyan-200/70' : ''
            }`}
            style={{
          ...
          <span
            className={`absolute top-full left-0 right-0 text-center whitespace-nowrap leading-[2] text-base transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(20%)] ${
              activeIndex === index ? 'opacity-100 [transform:translateY(20%)]' : 'opacity-0'
            }`}
          >
            {item.label}
          </span>
